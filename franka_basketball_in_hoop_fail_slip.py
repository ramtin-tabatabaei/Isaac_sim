"""
franka_basketball_in_hoop_fail_slip.py

Spawns a Franka Emika Panda arm, a ball, and a simple hoop, then executes a
failure variant of the basketball-in-hoop task inspired by:
    /home/ramtin/AHA/ttm_inspection_reports/001_basketball_in_hoop.llm_context.md
and the delayed gripper-release pattern in:
    /home/ramtin/AHA/aha/Data_Generation/rlbench-failgen/failgen/fail_slip.py

Run with:
    cd ~/IsaacLab
    ./isaaclab.sh -p scripts/franka_basketball_in_hoop_fail_slip.py
"""
...
# ----------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Franka basketball-in-hoop slip-failure task.")
...
# Failure injection: after this subgoal completes, wait a few simulator steps,
# then open the gripper and keep it open. This creates a deliberate slip/drop.
...
def main():
    ...
    sim.reset()
    print("[INFO]: Basketball slip-failure scene ready. Starting task.")
