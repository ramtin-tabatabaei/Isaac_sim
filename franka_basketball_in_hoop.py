...
EE_CAMERA_PRIM_NAME = "EndEffectorCamera"
EE_CAMERA_PATH = f"/World/envs/env_0/Robot/panda_hand/{EE_CAMERA_PRIM_NAME}"
EE_CAMERA_RESOLUTION = (640, 480)
EE_CAMERA_OFFSET_POS = (0.0, 0.0, 0.055)
EE_CAMERA_OFFSET_ROT = (0.0, 1.0, 0.0, 0.0)
EE_CAMERA_FOCAL_LENGTH = 18.0
EE_CAMERA_HORIZONTAL_APERTURE = 20.955
EE_CAMERA_CLIPPING_RANGE = (0.02, 10.0)
...
def _spawn_end_effector_camera(camera_path):
    stage = sim_utils.get_current_stage()
    if not stage.GetPrimAtPath(camera_path).IsValid():
        prim = sim_utils.create_prim(
            camera_path,
            "Camera",
            translation=EE_CAMERA_OFFSET_POS,
            orientation=EE_CAMERA_OFFSET_ROT,
        )
    else:
        prim = stage.GetPrimAtPath(camera_path)

    prim.GetAttribute("focalLength").Set(EE_CAMERA_FOCAL_LENGTH)
    prim.GetAttribute("focusDistance").Set(1.0)
    prim.GetAttribute("horizontalAperture").Set(EE_CAMERA_HORIZONTAL_APERTURE)
    prim.GetAttribute("verticalAperture").Set(
        EE_CAMERA_HORIZONTAL_APERTURE * EE_CAMERA_RESOLUTION[1] / EE_CAMERA_RESOLUTION[0]
    )
    prim.GetAttribute("clippingRange").Set(Gf.Vec2f(*EE_CAMERA_CLIPPING_RANGE))


def _set_viewport_camera(viewport, camera_path):
...
def _show_end_effector_camera_view(sim, camera_path):
    if not sim.has_gui():
        print(f"[INFO]: End-effector camera available at {camera_path} (run without --headless to view it).")
        return
    ...
    try:
        ...
        window = create_viewport_window(
            "End Effector Camera",
            width=EE_CAMERA_RESOLUTION[0],
            height=EE_CAMERA_RESOLUTION[1],
        )
        viewport = getattr(window, "viewport_api", None) or getattr(window, "viewport", None)
        if viewport is not None and _set_viewport_camera(viewport, camera_path):
            print(f"[INFO]: Opened an End Effector Camera viewport from {camera_path}.")
            return
    except Exception as exc:
        print(f"[WARN]: Could not open a separate end-effector viewport ({exc}).")
    ...
    try:
        ...
        if viewport is not None and _set_viewport_camera(viewport, camera_path):
            print(f"[INFO]: Main viewport is now showing the end-effector camera at {camera_path}.")
            return
    except Exception as exc:
        print(f"[WARN]: Could not switch the main viewport to the end-effector camera ({exc}).")

    print(f"[INFO]: End-effector camera spawned at {camera_path}; select it from the viewport camera menu.")
...
def main():
    ...
    _spawn_end_effector_camera(EE_CAMERA_PATH)
    sim.reset()
    _show_end_effector_camera_view(sim, EE_CAMERA_PATH)
